            ...
            foreach ($stacks as $key=>$stack) {
                ...
                if ((new InventoryManager)->debitStack($stack->user, 'Choice Box Opened', ['data' => ''], $stack, $data['quantities'][$key])) {
                    ...
                    if ($matches == [] || !isset($matches[1]) || !isset($matches[2])) {
                        throw new \Exception('Failed to get reward information.');
                    }

                    // Check that quantity information is set for the prize/that it's in the
                    // tag's data
                    if (!isset($stack->item->tag('choicebox')->data[$matches[1]][$matches[2]])) {
                        throw new \Exception('Failed to retrieve reward information.');
                    }
