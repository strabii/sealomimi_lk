        ...
        @php
            ...
            foreach($tag->data as $type=>$group) {
                switch($type) {
                    ...
                    case 'pets':
                        foreach($group as $id=>$quantity) {
                            $pet = App\Models\Pet\Pet::where('id', $id)->first();
                            $rewardOptions['Pets'][$type.'-'.$id] =  $pet->name.' x'.$quantity;
                        }
                        break;
                    case 'themes':
                        foreach($group as $id=>$quantity) {
                            $themes = App\Models\Theme::where('id', $id)->first();
                            $rewardOptions['Themes'][$type.'-'.$id] =  $themes->name.' x'.$quantity;
                        }
                        break;
                    case 'awards':
                        foreach($group as $id=>$quantity) {
                            $awards = App\Models\Award\Award::where('id', $id)->first();
                            $rewardOptions['Awards'][$type.'-'.$id] =  $awards->name.' x'.$quantity;
                        }
                        break;
                    case 'raffle_tickets':
                        foreach($group as $id=>$quantity) {
                    ...
                    case 'weapons':
                        foreach($group as $id=>$quantity) {
                            $weapons = App\Models\Claymore\Weapon::where('id', $id)->first();
                            $rewardOptions['Weapons'][$type.'-'.$id] =  $weapons->name.' x'.$quantity;
                        }
                        break;
                    case 'gears':
                        foreach($group as $id=>$quantity) {
                            $gears = App\Models\Claymore\Gear::where('id', $id)->first();
                            $rewardOptions['Gears'][$type.'-'.$id] =  $gears->name.' x'.$quantity;
                        }
                        break;
                }
            }
