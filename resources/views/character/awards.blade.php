            ...
            @foreach($categoryAwards->chunk(4) as $chunk)
                <div class="row mb-3">
                    @foreach($chunk as $awardId=>$stack)
                        <div class="col-sm-3 col-6 text-center awards-award" data-id="{{ $stack->first()->pivot->id }}" data-name="{{ $stack->first()->name }} {{ __('awards.award') }}"> <!--{{ $character->name ? $character->name : $character->slug }}'s-->
                            <div class="mb-1">
                            ...
                            <div>
                            <a href="#" class="awards-stack awards-stack-name"><!--@if($stack->first()->user_limit != 1) [x{{ $stack->sum('pivot.count') }}]@endif -->{{ $stack->first()->name }}</a>
                            </div>
                        </div>
                    @endforeach
                </div>
...
<div class="text-right">
    <a href="{{ url($character->url.'/'.__('awards.award').'-logs') }}">[view {{__('awards.awards')}} logs]</a>
</div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                {!! Form::open(['url' => 'admin/character/'.$character->slug.'/grant-awards']) !!}
                    ...
                    <div class="d-flex justify-content-end"><a href="#" class="btn btn-primary" id="add-award">Add {{ucfirst(__('awards.award'))}}</a></div>
                    ...
                    <h5 class="mt-2">Details (Optional)</h5>

                    <div class="form-group">
                        {!! Form::label('data', 'Log Reason') !!} {!! add_help('A reason for the grant. This will be noted in the logs and in the '.__('awards.award').'\'s description.') !!}
                        {!! Form::text('data', null, ['class' => 'form-control', 'maxlength' => 400]) !!}
                    </div>

                    <div class="form-group">
                        {!! Form::label('notes', 'Notes') !!} {!! add_help('Additional notes for the '.__('awards.award').'. This will appear in the '.__('awards.award').'\'s description, but not in the logs.') !!}
                        {!! Form::text('notes', null, ['class' => 'form-control', 'maxlength' => 1024]) !!}
                    </div>

                    <div class="form-group">
                        {!! Form::checkbox('disallow_transfer', 1, 0, ['class' => 'form-check-input', 'data-toggle' => 'toggle']) !!}
                        {!! Form::label('disallow_transfer', ucfirst(__('lorekeeper.character')).'-bound', ['class' => 'form-check-label ml-3']) !!} {!! add_help('If this is on, the '.__('lorekeeper.character').'\'s owner will not be able to transfer this '.__('awards.award').' to their user '.__('awards.awardcase').'. '.ucfirst(__('awards.awards')).' that disallow transfers by default will still not be transferrable.') !!}
                    </div>
