<div class="profile-assets-content">
    @if($character)
        <div>
            <a href="{{ $character->url }}"><img src="{{ isset($fullImage) && $fullImage ? $character->image->imageUrl : $character->image->thumbnailUrl }}" class="{{ isset($fullImage) && $fullImage ? '' : 'img-thumbnail' }}" style="{{ isset($fullImage) && $fullImage ? 'max-width:100%;' : '' }} {{ isset($limitHeight) && $limitHeight ? 'max-height:500px;' : '' }}" alt="{{ $character->fullName }}" /></a>
        </div>
        <div class="my-1">
    ...
    @else
        ...
        @if(Auth::check() && Auth::user()->id == $user->id)
            <div class="text-center"><a href="{{ '/characters' }}" class="btn btn-primary">View Your Characters</a></div>
        @else
