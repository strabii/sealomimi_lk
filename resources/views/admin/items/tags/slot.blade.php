...
<div class="form-group">
    ...
    @if ($isMyo)
        {!! add_help('This section is for making additional notes about the '.__('lorekeeper.myo').' slot. If there are restrictions for the '.__('lorekeeper.character').' that can be created by this slot that cannot be expressed with the options below, use this section to describe them.') !!}
    @else
...
<div class="form-group">
    ...
    {!! Form::label('is_visible', 'Is Visible', ['class' => 'form-check-label ml-3']) !!} {!! add_help(
        'Turn this off to hide the ' . ($isMyo ? __('lorekeeper.myo') : __('lorekeeper.character')) . '. Only mods with the Manage Masterlist power (that\'s you!) can view it - the owner will also not be able to see the ' . ($isMyo ? __('lorekeeper.myo') : __('lorekeeper.character')) . '\'s page.',
    ) !!}
</div>
...
<div class="alert alert-info">
    These are displayed on the {{ __('lorekeeper.myo') }} slot's profile, but don't have any effect on site functionality except for the following:
    <ul>
...
<div class="form-group">
    {!! Form::label(ucfirst(__('lorekeeper.subtype')).' (Optional)') !!} {!! add_help(
